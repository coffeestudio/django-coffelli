(function() {
    ...
    tinymce.PluginManager.requireLangPack('coffelli');
    var DOM = tinymce.DOM;
    
    tinymce.create("tinymce.plugins.Grappelli", {
        init: function(ed, url) {
            var t = this;
            tb = ed.getParam("coffelli_adv_toolbar", "toolbar2");
            documentstructure_css = url + "../../../themes/advanced/skins/coffelli/content_documentstructure_" + ed.settings.language + ".css";
            cookie_date = new Date();
            var year = cookie_date.getFullYear();
            cookie_date.setYear(year + 1);
            
            // get/set cookie
            cookie_coffelli_show_documentstructure = tinymce.util.Cookie.get('coffelli_show_documentstructure');
            if (cookie_coffelli_show_documentstructure != null) {
                ed.settings.coffelli_show_documentstructure = cookie_coffelli_show_documentstructure;
            } else {
                tinymce.util.Cookie.set('coffelli_show_documentstructure', ed.settings.coffelli_show_documentstructure, cookie_date, '/');
            }
            ...
            ed.onInit.add(function() {
                ...
                if (ed.settings.coffelli_adv_hidden) {
                    t._hide_adv_menu(ed);
                } else {
                    t._show_adv_menu(ed);
                }
                if (ed.settings.coffelli_show_documentstructure == "on") {
            ...
            ed.addButton("coffelli_adv", {
                title: "coffelli.coffelli_adv_desc",
                cmd: "Grappelli_Adv"
            });
            
            // ADD BUTTON: DOCUMENT STRUCTURE
            ed.addButton("coffelli_documentstructure", {
                title: "coffelli.coffelli_documentstructure_desc",
            ...
            ed.addShortcut("alt+shift+z", ed.getLang("coffelli_adv_desc"), "Grappelli_Adv");
        ...
        _show_documentstructure: function(ed) {
            ...
            ed.settings.coffelli_show_documentstructure = 'on';
            tinymce.util.Cookie.set('coffelli_show_documentstructure', 'on', cookie_date, '/');
            ed.controlManager.setActive('coffelli_documentstructure', true);
        },
        _hide_documentstructure: function(ed) {
            ...
            if (vs_link !== null) {
                ...
                ed.settings.coffelli_show_documentstructure = 'off';
                tinymce.util.Cookie.set('coffelli_show_documentstructure', 'off', cookie_date, '/');
                ed.controlManager.setActive('coffelli_documentstructure', false);
            }
        },
        ...
        _show_adv_menu: function(ed) {
            if (ed.controlManager.get(tb, false)) {
                ed.controlManager.setActive("coffelli_adv", 1);
                DOM.show(ed.controlManager.get(tb).id);
                this._resizeIframe(ed, tb, -28);
                ed.settings.coffelli_adv_hidden = 0;
            }
        },
        _hide_adv_menu: function(ed) {
            if (ed.controlManager.get(tb, false)) {
                ed.controlManager.setActive("coffelli_adv", 0);
                DOM.hide(ed.controlManager.get(tb).id);
                this._resizeIframe(ed, tb, 28);
                ed.settings.coffelli_adv_hidden = 1;
            }
        },
        ...
        getInfo: function() {
            return {
                ...
                infourl: "http://code.google.com/p/django-coffelli/",
                version: "1.1"
            }
        }
    ...
    tinymce.PluginManager.add("coffelli", tinymce.plugins.Grappelli)
