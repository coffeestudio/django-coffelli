...
(function($) {
    ...
    // dateformat
    coffelli.getFormat = function(type) {
        if (type == "date") {
    ...
    coffelli.cleanInputTypes = function() {
        $("form").each(function(){
    ...
    coffelli.initDateAndTimePicker = function() {
        ...
        var options = {
            ...
            buttonText: '',
            dateFormat: coffelli.getFormat('date'),
            ...
            beforeShow: function(year, month, inst) {
                coffelli.datepicker_instance = this;
            }
        };
        ...
        $(document).on('click', '.ui-datepicker-current', function() {
            $.datepicker._selectDate(coffelli.datepicker_instance);
            coffelli.datepicker_instance = null;
        });
    ...
    coffelli.initSearchbar = function() {
        var searchbar = $("input.grp-search-field");
        searchbar.focus();
    };
    
    coffelli.updateSelectFilter = function(form) {
    ...
    coffelli.reinitDateTimeFields = function(form) {
    ...
    coffelli.get_app_label = function(elem) {
    ...
    coffelli.get_model_name = function(elem) {
        var link = elem.next("a");
        if (link.length > 0) {
            var url = link.attr('href').split('/');
    ...
    coffelli.get_query_string = function(elem) {
        var link = elem.next("a");
        if (link.length > 0) {
            var url = link.attr('href').split('/');
